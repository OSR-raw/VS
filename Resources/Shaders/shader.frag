...
#ifdef Lightning
	out_Color.rgb *= sh_light(normal, groove);// * 0.5;
	//vec3 light_dir = normalize(light - position);
	//out_Color.rgb = out_Color.rgb*max(dot(normal, light_dir), 0.1);
	/*{
	//out_Color.rgb = position/length(position);
		vec3 light_dir = normalize(light - position);
		float diff = clamp(dot(normal, light_dir), 0.1, 1.0);
		//out_Color.rgb = out_Color.rgb*diff;
		float sat = pow(clamp(dot(normal, lightHalfDir),0.0,1.0), 20.0);

		float scalar = clamp(length(normal) / (0.5 * 0.5 * 0.5), 0.1, 1.0);
		out_Color.rgb = out_Color.rgb * vec3(1.0,1.0,1.0) * clamp(diff + sat, 0.2, 2.0) * scalar;			
	}*/
#endif		
}